...
GLOBAL_LIST_AS(heretic_path_datums, init_heretic_path_datums())
...
/proc/init_heretic_path_datums()
	var/list/paths = list()
	for(var/datum/heretic_knowledge_tree_column/column_path as anything in subtypesof_real(/datum/heretic_knowledge_tree_column))
	...
	for(var/draft in drafts)
		...
		for(var/cycle in 1 to 3)
			...
			if(guaranteed_draft && cycle == 1)
				...
				var/chosen_tier = min(text2num(pickweight(probabilities)), length(elligible_knowledge))
		...
		var/list/blacklist_ids = list_values(draft_blacklist)
