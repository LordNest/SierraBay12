...
/datum/heretic_knowledge/hunt_and_sacrifice/recipe_snowflake_check(mob/living/carbon/human/user, list/atoms, list/selected_atoms, turf/loc)
	...
	to_chat(user, SPAN_WARNING( "ritual failed, no sacrifice found!"))
	return FALSE
...
/datum/heretic_knowledge/hunt_and_sacrifice/on_finished_recipe(mob/living/user, list/selected_atoms, turf/loc)
	...
	if(!LAZYLEN(heretic_datum.sac_targets) && !iscultist(sac))
		if(obtain_targets(user, heretic_datum = heretic_datum))
			...
			to_chat(user, SPAN_WARNING( "ritual failed, no targets found!"))
			return FALSE
...
/datum/heretic_knowledge/hunt_and_sacrifice/proc/obtain_targets(mob/living/user, silent = FALSE, datum/antagonist/heretic/heretic_datum)
	...
	if(!length(valid_targets))
		if(!silent)
			to_chat(user, SPAN_WARNING("No sacrifice targets could be found!"))
		return FALSE
	...
	for(var/datum/mind/sec_mind as anything in shuffle(valid_targets))
		if(sec_mind.assigned_job.department_flag == SEC)
	...
	for(var/datum/mind/chosen_mind as anything in final_targets)
		...
		if(!silent)
			to_chat(user, SPAN_DANGER("[chosen_mind.current.real_name], the [chosen_mind.assigned_job.title]."))
...
/datum/heretic_knowledge/hunt_and_sacrifice/proc/sacrifice_process(mob/living/user, list/selected_atoms, turf/loc)
	...
	for(var/datum/antagonist/heretic/all_heretic in GLOB.all_antag_types_)
		all_heretic.remove_sacrifice_target(sacrifice)

	var/feedback = "Your patrons accept your offer"
	var/sac_job_flag = sacrifice.mind?.assigned_job?.head_position
	var/datum/antagonist/cultist/cultist_datum = iscultist(sacrifice)
	...
	if(cultist_datum)
		...
		feedback += " <i>laughing at the fate of Nar-Sie follower</i>"
		// easier to read
		...
		if(prob(min(15 * rewards_given)) && (rewards_given <= 5))
			for(var/datum/mind/mind in GLOB.cult.current_antagonists)
				if(mind.current)
					var/msg = SPAN_OCCULT("A vile heretic has sacrificed one of our own. Destroy and sacrifice the infidel before it claims more!")
					to_chat(mind.current, msg)
			// he(retic) gets a warn too
			to_chat(user, SPAN_OCCULT("How DARE you!? I will see you destroyed for this."))
			var/non_flavor_warning = SPAN_DANGER("You feel that your action has attracted unwanted attention.")
			to_chat(user, non_flavor_warning)
		return
	else
		heretic_datum.adjust_knowledge_points(2)

	to_chat(user, SPAN_OCCULT("[feedback]."))
...
/datum/heretic_knowledge/hunt_and_sacrifice/proc/begin_sacrifice(mob/living/carbon/human/sac_target)
	...
	var/datum/antagonist/heretic/our_heretic = isheretic(heretic_mind)
	if(!our_heretic)
	...
	var/obj/landmark/heretic/destination_landmark = GLOB.heretic_sacrifice_landmarks[our_heretic.heretic_path] || GLOB.heretic_sacrifice_landmarks[PATH_START]
	...
	// sac_target.equip_to_slot_or_del(new /obj/item/handcuffs/cult, slot_handcuffed_str)

	sac_target.adjustBrainLoss(15)
	sac_target.do_jitter()
	log_attack(heretic_mind.current, sac_target, "sacrificed")

	addtimer(new Callback(sac_target, TYPE_PROC_REF(/mob/living/carbon, do_jitter)), SACRIFICE_SLEEP_DURATION * (1/3))
	addtimer(new Callback(sac_target, TYPE_PROC_REF(/mob/living/carbon, do_jitter)), SACRIFICE_SLEEP_DURATION * (2/3))

	// If our target is dead, try to revive them
	// and if we fail to revive them, don't proceede the chain
	sac_target.adjustOxyLoss(-100)
	if(sac_target.is_dead())
		return

	// if(sac_target.AdjustUnconscious(SACRIFICE_SLEEP_DURATION))
	//	to_chat(sac_target, SPAN_OCCULT("Your mind feels torn apart as you fall into a shallow slumber..."))
	// else
	//	to_chat(sac_target, SPAN_OCCULT("Your mind begins to tear apart as you watch dark tendrils envelop you."))

	// sac_target.AdjustParalyzed(SACRIFICE_SLEEP_DURATION * 1.2)
	// sac_target.AdjustImmobilized(SACRIFICE_SLEEP_DURATION * 1.2)

	addtimer(new Callback(src, PROC_REF(after_target_sleeps), sac_target, destination), SACRIFICE_SLEEP_DURATION * 0.5) // Teleport to the minigame
...
/datum/heretic_knowledge/hunt_and_sacrifice/proc/after_target_sleeps(mob/living/carbon/human/sac_target, turf/destination)
	...
	if(!destination || !do_teleport(sac_target, destination))
		disembowel_target(sac_target)
		return
	...
	to_chat(sac_target, SPAN_OCCULT("Unnatural forces begin to claw at your every being from beyond the veil."))

	playsound(sac_target, 'mods/heretic/sound/heretic_sacrifice.ogg', 50, FALSE) // play theme

	// sac_target.apply_status_effect(/datum/status_effect/unholy_determination, SACRIFICE_REALM_DURATION)
	addtimer(new Callback(src, PROC_REF(after_target_wakes), sac_target), SACRIFICE_SLEEP_DURATION * 0.5) // Begin the minigame
...
/datum/heretic_knowledge/hunt_and_sacrifice/proc/curse_organs(mob/living/carbon/human/sac_target)
	...
	for (var/i in 1 to total_implant)
		...
		to_give.replaced(sac_target)

	new /obj/gibspawner/human(get_turf(sac_target), sac_target)
	sac_target.visible_message(SPAN_WARNING("Several organs force themselves out of [sac_target]!"))
...
/datum/heretic_knowledge/hunt_and_sacrifice/proc/after_target_wakes(mob/living/carbon/human/sac_target)
	...
	sac_target.Stun(5)
	sac_target.Weaken(3)
	sac_target.do_jitter(2)
	sac_target.eye_blurry += rand(15,20)
	sac_target.dizziness += rand(15,20)
	sac_target.emote("scream")

	to_chat(sac_target, SPAN_OCCULT("The grasp of the Mansus reveal themselves to you!"))
	to_chat(sac_target, SPAN_WARNING("You feel invigorated! Fight to survive!"))
	// When it runs out, let them know they're almost home free
	addtimer(new Callback(src, PROC_REF(after_helgrasp_ends), sac_target), helgrasp_time)
	// Win condition
	var/win_timer = addtimer(new Callback(src, PROC_REF(return_target), sac_target), SACRIFICE_REALM_DURATION, TIMER_STOPPABLE)
...
/datum/heretic_knowledge/hunt_and_sacrifice/proc/after_helgrasp_ends(mob/living/carbon/human/sac_target)
	...
	to_chat(sac_target, SPAN_DANGER("The worst is behind you... Not much longer! Hold fast, or expire!"))
...
/datum/heretic_knowledge/hunt_and_sacrifice/proc/return_target(mob/living/carbon/human/sac_target)
	...
	if(isheretic(sac_target))
		var/datum/antagonist/heretic/victim_heretic = isheretic(sac_target)
	...
	if((get_z(below_target) in GLOB.using_map.station_levels))
		return

	// Teleport them to a random safe coordinate on the station z level.
	var/turf/simulated/safe_turf = pick_area_turf_in_connected_z_levels(list(GLOBAL_PROC_REF(is_not_space_area)), list(GLOBAL_PROC_REF(not_turf_contains_dense_objects), GLOBAL_PROC_REF(IsTurfAtmosSafe)), 1)
	do_teleport(sac_target, safe_turf)
	if(!safe_turf)
		stack_trace("[type] - return_target was unable to find a turf for [sac_target] to return to.")
	...
	if(heretic_mind?.current)
		...
		if(sac_target.stat == DEAD)
			composed_return_message += SPAN_BAD("dead. ")
		else
			composed_return_message += SPAN_GOOD("alive, but with a shattered mind. ")

		composed_return_message += SPAN_NOTICE("You hear a whisper... ")
		composed_return_message += SPAN_OCCULT(get_area_name(safe_turf, TRUE))
...
/datum/heretic_knowledge/hunt_and_sacrifice/proc/after_return_live_target(mob/living/carbon/human/sac_target)
	to_chat(sac_target, SPAN_OCCULT("The fight is over, but at great cost. You have been returned to the station in one piece."))
	if(isheretic(sac_target))
		to_chat(sac_target, SPAN_DANGER("You don't remember anything leading up to the experience, but you feel your connection with the Mansus weakened - Knowledge once known, forgotten..."))
	else
		to_chat(sac_target, SPAN_DANGER("You don't remember anything leading up to the experience - All you can think about are those horrific hands..."))
...
/datum/heretic_knowledge/hunt_and_sacrifice/proc/after_return_dead_target(mob/living/carbon/human/sac_target)
	to_chat(sac_target, SPAN_OCCULT("You failed to resist the horrors of the Mansus! Your ruined body has been returned to the station."))
	to_chat(sac_target, SPAN_OCCULT("The experience leaves your mind torn and memories tattered. You will not remember anything leading up to the experience if revived."))

	var/obj/psi_plane/psinomaly/illusion = new(get_turf(sac_target))
	illusion.name = "\improper weakened psionic breach"
	illusion.desc = "A rift wide enough for something... or someone... to come through."
	illusion.aura_color = COLOR_NT_RED
...
/datum/heretic_knowledge/hunt_and_sacrifice/proc/disembowel_target(mob/living/carbon/human/sac_target)
	if(heretic_mind)
		log_attack(heretic_mind.current, sac_target, "disemboweled via sacrifice")
	// sac_target.spill_organs(DROP_ALL_REMAINS)
	sac_target.apply_damage(250, DAMAGE_BRUTE)
	if(sac_target.stat != DEAD)
		sac_target.investigate_log("has been killed by heretic sacrifice.", "deaths")
		sac_target.death()
	sac_target.visible_message(
		SPAN_DANGER("[sac_target]'s organs are pulled out of [sac_target]'s chest by shadowy hands!"),
		SPAN_DANGER("Your organs are violently pulled out of your chest by shadowy hands!")
	)
